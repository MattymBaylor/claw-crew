...
class CrewConfig:
    ...
    def directory_block(
        ...
        lines += [
            ...
            "- To convene: ONE specialist per message. @-mention them and include",
            "  the marker CONFER(3): <one specific question>. Separate specialists =",
            "  separate messages.",
            "- Receiving a CONFER(n)? ANSWER IT YOURSELF — your piece only, in one",
            "  message, addressed to the asker, prefixed CONFER(n-1):. NEVER repeat",
            "  or re-send anyone else's questions, NEVER re-convene people who were",
            "  already convened, NEVER raise the number.",
